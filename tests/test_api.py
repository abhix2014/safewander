...
class TestApi:
    ...
    def setup_class(cls):
        ...
        for _ in range(40):
            try:
                status, _ = cls.client.get("/api/health")
                if status == 200:
                    break
            except Exception:
                time.sleep(0.1)
    ...
    def test_health_legacy_and_v1(self):
        status, legacy = self.client.get("/api/health")
        assert status == 200
        assert legacy["status"] == "ok"

        status, v1 = self.client.get("/api/v1/health")
        assert status == 200
        assert v1["status"] == "ok"

    def test_posts_list_and_create(self):
        status, posts = self.client.get("/api/posts?limit=2")
        assert status == 200
        assert len(posts["posts"]) == 2

        status, create = self.client.post(
            "/api/v1/posts",
        ...
        status, latest = self.client.get("/api/posts?limit=1")
        assert status == 200
    ...
    def test_sos_and_incidents(self):
        status, created = self.client.post("/api/sos", {"name": "SOS User", "lat": 30.0869, "lng": 78.2676})
        assert status == 201
        assert created["status"] == "alerting_nearby"

        status, incidents = self.client.get("/api/v1/incidents?limit=1")
        assert status == 200
